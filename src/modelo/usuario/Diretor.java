...
public class Diretor<ContaPoupanca> extends Funcionario {
	...
	public ArrayList<String> relatorioDiretor(Map<String, ContaCorrente> contaCorrente, Map<String, ContaPoupanca> contaPoupanca) {
		...
		for (String c : cp) {
			...
			String linha = ((Conta) ((Map<String, ContaPoupanca>) contaPoupanca).get(c)).getTitular().getNome() + " - "
					+ ((Conta) ((Map<String, ContaPoupanca>) contaPoupanca).get(c)).getTitular().formatCpf() + " - "
					+ ((Conta) ((Map<String, ContaPoupanca>) contaPoupanca).get(c)).getIdAgencia().getNumeroAgencia();
